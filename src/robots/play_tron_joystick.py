...
class OnnxController:
  """ONNX controller for the tron a1."""
  ...
  def get_obs(self, model, data) -> np.ndarray:
    ...
    if not self.joy_queue is None:
      try:
        ...
        command = -np.array([self.latest_axes[1] * 1., self.latest_axes[0] * 0.6, self.latest_axes[3] * 1.])
...
def load_callback(model=None, data=None):
  ...
  policy = OnnxController(
      policy_path=(_ONNX_DIR / "tron_a1_policy.onnx").as_posix(),
