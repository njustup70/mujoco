...
class AccMPCPathFollower:
    ...
    def setup_acados_model(self) -> AcadosModel:
        ...
        # 旋转矩阵 (Body to Global)
        rot_mat = vertcat(
            vertcat(cos(theta), -sin(theta)).T,
            vertcat(sin(theta),  cos(theta)).T
        )
        
        # 动力学方程向量化表达
        dp = rot_mat @ v_body
        d_angle = vertcat(omega, u[2])
        # dvx = ax + omega*vy, dvy = ay - omega*vx
        coriolis = vertcat(omega * x[4], -omega * x[3])
        dv = u[0:2] + coriolis

        model.f_expl_expr = vertcat(dp, d_angle[0], dv, d_angle[1])
        model.x = x
        model.u = u
        return model
    ...
    def update(self, x_current: np.ndarray) -> np.ndarray:
        ...
        if self.is_following_path:
            ...
            for k in range(self.n_horizon):
                s_k = s_start + self.ref_speed * self.dt * k
                ref_pos = self.path_planner.get_state_by_s(s_k)
                
                # 显式构造 9 维 yref: [states(6), controls(3)]
                yref = np.zeros(9, dtype=np.float64)
                yref[0:2] = ref_pos[0:2]  # 期望位置
                yref[2] = self.target_yaw
                # vx, vy, omega, ax, ay, alpha 默认为 0
                self.solver.set(k, "yref", yref)
            
            # 终端参考 yref_e (6维)
            yref_e = np.zeros(6, dtype=np.float64)
            yref_e[0:2] = self.path_planner.get_state_by_s(s_start + self.ref_speed * self.dt * self.n_horizon)[0:2]
            yref_e[2] = self.target_yaw
